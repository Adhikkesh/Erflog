...
def interviewer_node(state: InterviewState) -> dict:
    ...
    print(f"[Chat Interviewer] Stage: {stage}, Turn: {turn}, StageTurn: {stage_turn}, Ending: {state.get('ending', False)}")
    ...
    if stage_turn >= config["turns"]:
        ...
        if next_idx > current_idx:
            print(f"[Chat Interviewer] ✅ TRANSITIONING: {stage} -> {next_stage} (StageTurn {stage_turn}/{config['turns']})")
            stage = next_stage
            stage_turn = 0
            if next_stage == "end":
    ...
    if stage == "end" or state.get("ending", False) or turn >= MAX_TURNS:
        print(f"[Chat Interviewer] Triggering conclusion - Stage:{stage}, Ending:{state.get('ending')}, Turn:{turn}/{MAX_TURNS}")
        prompt = get_stage_prompt("conclusion", ctx, 1) + " Final message."
    ...
    print(f"[Chat Interviewer] Output - Stage: {new_state.get('stage', stage)}, Turn: {new_state['turn']}, StageTurn: {new_state['stage_turn']}")
    return new_state
...
def check_stage_transition(state: InterviewState) -> dict:
    ...
    config = STAGES.get(stage, {"turns": 2, "next": "end"})
    print(f"[Transition] Stage: {stage}, StageTurn: {stage_turn}/{config['turns']}")
    ...
    if stage_turn >= config["turns"]:
        ...
        if next_idx > current_idx:
            ...
            if next_stage == "end":
                updates["ending"] = True
            print(f"[Transition] Advancing: {stage} -> {next_stage} (Turn {turn})")
            return updates
        else:
            print(f"[Transition] BLOCKED backwards movement from {stage} to {next_stage}")
    
    print(f"[Transition] Staying in {stage}")
    return {}

def should_continue(state: InterviewState) -> Literal["continue", "evaluate"]:
    stage = state.get("stage")
    ending = state.get("ending")
    decision = "evaluate" if (stage == "end" or ending) else "continue"
    print(f"[ShouldContinue] Stage: {stage}, Ending: {ending} -> {decision}")
    return decision
